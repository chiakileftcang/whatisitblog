...
;
	...
	$array[]=array(name=>"local",title=>__('生成在网站文件目录', 'reallystatic'),main=>'<div style="margin-left:50px;"><table border="0" width="100%"><tr><td valign="top" width="350px">'.__('缓存静态文件的路径', 'reallystatic').'</td><td>:<input name="rs_lokalerspeicherpfad" size="50" type="text" value="' . loaddaten ( "rs_lokalerspeicherpfad" , 'reallystatic') . '"	/> '.rs_fragehelp(__('the path inside your system e.g. "/www/html/".If it should saved to maindirectory write "/" ', 'reallystatic')).'</td></tr>'.$sub.'</table></div><br>');
	return $array;
...
function rs_local_writecontent($ziel, $content) {
  if(!rs_hostlinux()){$ziel=str_replace(array("\\\\","/"),array("\\","\\"),$ziel);}
	...
	if ($fh === false) {
		...
		$fh = @fopen ( $ziel, 'w+' );
		if ($fh === false) {
			do_action ( "rs-error", "文件夹未生成成功", $dir,$ziel );
